function highlightNavbar() {
    // Highlight navbar
    $('#event-planner-nav').addClass('active');
}
...
function createRecUnitCheckHtml(size) {
    var imgHtml = $('<img></img>');
    imgHtml.attr('src', '/event-planner/assets/img/rec_unit_check.png');
...
function createBoosterV2El(id, unit) {
    var imgDiv = $('<div></div>');
    imgDiv.append(createImgHtml(getThumb(id), 40, false));
    ...
    imgDiv.data('max_lv', unit.maxLevel);
    imgDiv.data('team', -1);
    imgDiv.attr('id', 'booster_v2_' + id);
...
function renderBoostersV2() {
    ...
    // Remember in-place don't-have marks before rebuild
    var inPlaceDontHave = {};
    $pool.children('.booster.assigned-dh').each(function () {
        inPlaceDontHave[$(this).data('id')] = true;
    });
...
function findBoostersByUnitId(unitId) {
    return $('.booster').filter(function () {
        return String($(this).data('id')) === String(unitId)
            && !$(this).hasClass('booster-clone')
            && !$(this).hasClass('non-booster');
    });
}

function findBoosterByUnitId(unitId) {
    var $tm = $('#booster_' + unitId);
    if ($tm.length) return $tm;
    var $v2 = $('#booster_v2_' + unitId);
...
function populateUnitModal(src, selectedId, assigned) {
    ...
    $('#non-booster-name-div').hide();
    $("#non-booster-unit").val("");
    $("#non-booster-name").val("");
    $("#non-booster-name-results").empty().hide();
    ...
    if (src) {
        ...
        $('#non-booster-name-div').css('display', 'flex');
...
function getUnitSpecial(unitId) {
    ...
    if (unitDetail) {
        ...
        } else if (typeof spDesc === 'string') {
            special.push(spDesc);
        }
        // else: unsupported special shape — leave empty (unit fails ability filters)
    }
...
function regexTestHelper(text, regex, subType) {
    ...
    if (typeof text !== 'string') {
        // Special/captain data can be nested objects — skip non-strings safely
        if (typeof text === 'object')
            return false;
        text = String(text);
    }

    // Avoid /g lastIndex bugs across repeated calls
    ...
    if (match) {
        for (var i in subType.groups) {
            var subText = match[i];
            if (subText != null && subType.regex) {
                if (subType.regex.global)
                    subType.regex.lastIndex = 0;
...
$(document).ready(function () {
    ...
    if (getUrlParameter('transfer')) {
        ...
        if ((serverTmp == 'glb' || serverTmp == 'jpn') && init(tmId, serverTmp, true)) {
            ...
            if (dontHaveStr) {
                ...
                for (var i = 0; i < dontHaves.length; i++) {
                    ...
                    if (unitId !== 0) {
                        markDontHaveBoosters(unitId, dontHaveMode == 0);
                    }
                }
            }
        ...
        // Reset browser url
        history.replaceState('', '', '/event-planner/');
    } else {
        // Retrieve TM ID
        tmId = getUrlParameter('tmId');
    ...
    $(document).on('click', '.select-modal-unit', function () {
        ...
        var b = findBoosterByUnitId(unitId);
            ...
            if (op.guide) {
                for (var gi in op.guide) {
                    ...
                    for (var di in g.detail) {
                        ...
                        decorateGuideAction(d.action, guideStageTypeId, guideStageTypeClone, tmId);
    ...
    var ids = ["booster_2_5x", "booster_2_3x", "booster_2_25x", "booster_2_2x", "booster_2_2x_v2", "booster_2_1x", "booster_2x",
        ...
        "booster_1_2x_tm", "booster_1_2x_other", "booster_1_1x", "booster_1x", "booster_v2"];

    for (var id of ids) {
        var boosterEl = document.getElementById(id);
        if (!boosterEl) continue;
    ...
    new Sortable(dontHaveEl, {
        group: {
            ...
            pull: true,
            put: ['dont-have-group', 'booster-group']
        },
        onAdd: function (evt) {
            var item = $("#" + evt.item.id);
            ...
            // Sync other copies of the same character (TM pool / V2)
            findBoostersByUnitId(unitId).not(item).each(function () {
                var $b = $(this);
                $b.addClass('assigned-dh');
                $b.data('team', -1);
                if (dontHaveMode == 0 || dontHaveMode == '0') {
                    $b.detach().css({
                        top: 0,
                        left: 0
                    }).insertBefore($('#add-button'));
    ...
    function searchUnitsByName(query) {
        var unitsDb = window.units || {};
        ...
        Object.keys(unitsDb).forEach(function (key) {
            var u = unitsDb[key];
            if (!u || !u.name) return;
            if (String(u.name).toLowerCase().indexOf(q) >= 0)
                results.push(u);
        });
    ...
    function appendNonBoosterNameHits(units) {
        var $box = $('#non-booster-name-results');
        units.forEach(function (u) {
            var $item = $('<button type="button" class="list-group-item list-group-item-action non-booster-name-hit"></button>');
            $item.attr('data-id', u.id);
            $item.attr('data-name', u.name);
            $item.append(createImgHtml(getThumb(u.id), 40, false));
            var $label = $('<span class="non-booster-name-label"></span>');
            $label.append(
                $('<span></span>').addClass(u.type || '').text('#' + u.id)
            );
            $label.append(document.createTextNode(' ' + u.name));
            $item.append($label);
            $box.append($item);
        });
    }

    function loadMoreNonBoosterNameResults() {
        var state = nonBoosterNameState;
        if (state.offset >= state.matches.length) return;

        var next = state.matches.slice(state.offset, state.offset + state.pageSize);
        state.offset += next.length;
        appendNonBoosterNameHits(next);

        var $box = $('#non-booster-name-results');
        $box.find('.non-booster-name-more').remove();
        if (state.offset < state.matches.length) {
            $box.append(
                $('<div class="list-group-item non-booster-name-more text-muted text-center"></div>')
                    .text('Scroll for more (' + state.offset + '/' + state.matches.length + ')')
            );
        }
    }

    function renderNonBoosterNameResults(query) {
        var $box = $('#non-booster-name-results');
        var q = String(query || '').trim();
        nonBoosterNameState.query = q;
        nonBoosterNameState.matches = searchUnitsByName(q);
        nonBoosterNameState.offset = 0;
        $box.empty();

        if (!nonBoosterNameState.matches.length) {
            $box.hide();
            return;
        }

        loadMoreNonBoosterNameResults();
        $box.show();
        $box.scrollTop(0);
    }

    $('#non-booster-name').on('input', function () {
        renderNonBoosterNameResults($(this).val());
    });

    $('#non-booster-name-results').on('scroll', function () {
        var el = this;
        if (el.scrollTop + el.clientHeight >= el.scrollHeight - 24)
            loadMoreNonBoosterNameResults();
    });

    $(document).on('click', '.non-booster-name-hit', function () {
        var unitId = $(this).attr('data-id');
        var unitName = $(this).attr('data-name') || '';
        $('#non-booster-unit').val(unitId);
        $('#non-booster-name').val(unitName);
        $('#non-booster-name-results').empty().hide();
        $('#non-booster-form').submit();
    });

    $(document).on('click', function (e) {
        if (!$(e.target).closest('.non-booster-name-wrap').length)
            $('#non-booster-name-results').hide();
    });
    ...
    $("#non-booster-form").submit(function (evt) {
        ...
        // Is booster (TM pool or getBoostersV2)
        var b = findBoosterByUnitId(unitId);
        if (b.length > 0) {
            from_list = b.parent();
